...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.registration.users = self.user_data
...
class StartupFrame(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(
            self,
            text="Welcome to the Mobile Food Delivery App",
            font=("Arial", 16),
        ).pack(pady=30)
...
class RegisterFrame(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text="Register New User", font=("Arial", 14)).pack(
            pady=20
        )
    ...
    def register_user(self):
        ...
        confirm = self.conf_pass_entry.get()

        result = self.master.registration.register(email, password, confirm)
        if result["success"]:
...
class MainAppFrame(tk.Frame):
    def __init__(self, master, user_email):
        ...
        self.results_tree = ttk.Treeview(
            self,
            columns=("cuisine", "location", "rating"),
            show="headings",
        )
        ...
        tk.Button(
            ...
            command=self.view_all_restaurants,
...
class AddItemPopup(tk.Toplevel):
    def __init__(self, master, menu, cart):
        ...
        self.item_var.set(
            self.menu.available_items[0]
            if self.menu.available_items
            else ""
        )
        tk.OptionMenu(
            self,
            self.item_var,
            *self.menu.available_items
        ).pack(pady=5)
...
class CheckoutPopup(tk.Toplevel):
    def __init__(self, master, order_placement):
        ...
        tk.Label(
            self,
            text=f"Subtotal: ${total['subtotal']:.2f}"
        ).pack()
        tk.Label(
            self,
            text=f"Tax: ${total['tax']:.2f}"
        ).pack()
        tk.Label(
            self,
            text=f"Delivery Fee: ${total['delivery_fee']:.2f}"
        ).pack()
        tk.Label(
            self,
            text=f"Total: ${total['total']:.2f}"
        ).pack()
        ...
        tk.Label(
            self,
            text="For credit card enter a 16-digit card number:",
        ).pack(pady=5)
